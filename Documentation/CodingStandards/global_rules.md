...
- The core library source is in `source/`.
- Platform-specific applications are in `platforms/`.
- Documentation is in `docs/` and `Documentation/`.
- Tests are in `test/`.
- External tools and third-party code are in `extra/`.

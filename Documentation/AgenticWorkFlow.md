...
Note: Tier 3 goals files (goals1.md, goals2.md, etc.) are created as needed when milestones are defined. See [GoalsTemplate.md](GoalsTemplate.md) for the template.
...
- The assistant should be concise and direct, using minimal output tokens.
- It has access to file tools (read, write, edit, glob, grep), a shell for running commands, and web tools.
- It must not guess URLs or commit secrets.
- It should follow the repository's code conventions first, then any framework/library conventions.
- When making changes, it should check for existing patterns in the codebase rather than assuming libraries are available.
- Do not add explanatory comments unless asked.
- Do not create new files unless explicitly required.
- After completing a task, run project-specific build/test commands if available.
- Confirm with the user before committing changes.
